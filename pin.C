...
int main(int argc, char * argv[]) {
	...
	ComponentTcad2d cmp;
//  cmp.EnableDebugging();
  cmp.Initialise("n3469_des.grd", "n3469_des.dat");
...
*/
  ...
  if (plotWeightingField) {
	...
	const int nSignalBins = 50000;
	const double totaltime = 10;
	const double tStep = totaltime / nSignalBins;
  ...
  std::ofstream output_charge, output_time;
	std::ofstream log;
	output_charge.open("charge_pin.dat", std::ios::out);
	output_time.open("time_pin.dat", std::ios::out);
	log.open("log_pin", std::ios::out);
	...
	for (unsigned int j = 0; j < nEvents; ++j) {
		...
		output_charge << nesum * ElementaryCharge << "\n";
		sensor.ConvoluteSignals();

		if (writeSignal) {
			char filename[50];
			sprintf(filename, "signal_pin_%05d.txt", j);
